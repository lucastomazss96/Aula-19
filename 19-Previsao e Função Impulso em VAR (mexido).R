...
pacotes <- c("forecast","dplyr","vars","urca","pwt8.0")
...
suppressMessages(require(pwt8.0))

data("pwt8.0")                                 #Carrega os dados elencados "pwt8.0" disponíveis no pacote
View(pwt8.0)                                   #Visualiza os dados na tabela pwt8.0

br1 <- subset(pwt8.0, country=="Brazil", select = c("rgdpna","emp","xr","ctfp","hc"))  #Cria a tabela "br" com dados das linhas que assumem o valor "country" (país) igual a "Brazil", selecionando as colunas cujas variáveis são "rgdpna" (PIB), "avh" (TRABALHO)  e "xr" (CÂMBIO)
...
br <- br[1:61,]
colnames(br) <-  c("PIB","Emprego","Cambio","PTF","KHumano")   #Renomeia as colunas para PIB, Trabalho, Câmbio, PTF e Capital humano
BR <- br[45:61,1:5]
...
plot(BRA,main="Variação do PIB, Emprego, Câmbio, PTF, Capital Humano no Brasil",type="o", col=c("Blue","Black","Red","Green","Purple"), plot.type="single")
